...
- [ ] API /api/bookings/calendar trả events theo date range + room filter
- [ ] Calendar hiển thị 3 chế độ: ngày, tuần, tháng
- [ ] Events mã màu theo status (vàng=pending, xanh=approved)
- [ ] Click slot trống → mở quick booking modal
- [ ] Click event → hiển thị popover chi tiết
- [ ] Room filter hoạt động (lọc events theo phòng)
- [ ] Chuyển tháng/tuần → auto re-fetch events
- [ ] Calendar responsive (hoạt động trên tablet)
- [ ] Locale tiếng Việt (tên thứ, tên tháng)
- [ ] Quick booking modal → tạo booking thành công → calendar refresh
