...
- [X] POST /api/bookings tạo booking thành công, status = pending
- [X] Kiểm tra trùng lịch: trả 409 kèm danh sách conflicts
- [X] Validation: quá khứ, giờ hành chính, thời lượng min/max, đặt trước 30 ngày
- [X] PATCH /approve: chỉ approver/admin, chỉ booking pending
- [X] PATCH /reject: kèm rejectionReason
- [X] PATCH /cancel: chủ booking hoặc admin, chỉ pending/approved
- [X] GET /api/bookings: filter + phân trang hoạt động
- [X] User thường chỉ thấy booking của mình

### Frontend

- [X] Form đặt phòng: chọn thời gian → load phòng trống → chọn phòng → submit
- [X] Hiển thị lỗi validation inline rõ ràng
- [X] Hiển thị conflict alert khi trùng lịch
- [X] Danh sách booking: filter + phân trang + status badges
- [X] Nút Duyệt/Từ chối/Hủy hiển thị đúng theo role và status
- [X] Modal nhập lý do từ chối
- [X] Chi tiết booking hiển thị đầy đủ info
- [X] Toast notifications cho mọi action
