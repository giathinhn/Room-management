...
async function main() {
  ...
  const history = historyRows
    .filter(m => !m.content.includes('Xin lỗi, tôi đang gặp sự cố kết nối'))
...
- Không tự ý tạo booking — chỉ đề xuất (propose_booking), để user xác nhận`;
  ...
  try {
    ...
    console.log('Raw text response length:', text ? text.length : 0);
    console.log('--- Raw text response start ---');
    console.log(text);
